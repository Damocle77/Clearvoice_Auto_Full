...
#
# USO BASE:
#   ./clearvoice_preset.sh [PRESET] [CODEC] [BITRATE] [FILES/DIRS]
#
# PRESET DISPONIBILI:
#   --film     : Ottimizzato per film con compressione mirata e maggior dettaglio vocale.
#                Parametri: VOICE_VOL=7.4, LFE=0.29, SURR=3.5, COMP=0.32:1.3:40:400
#                Filtri FC (non-DTS): Highpass 110Hz, Lowpass 8000Hz, Compressore, Softclipper.
#                (DTS Film aggiunge EQ specifici)
#
#   --serie    : Bilanciato per serie TV con compressione media e ottima intelligibilità.
#                Parametri: VOICE_VOL=7.3, LFE=0.29, SURR=3.4, COMP=0.28:1.25:55:350
#                Filtri FC (non-DTS): Highpass 100Hz, Lowpass 7800Hz, Compressore, Softclipper. 
#
#   --cartoni  : Leggero per cartoni animati con compressione minima per colonne sonore.
#                Parametri: VOICE_VOL=6.8, LFE=0.30, SURR=3.6, COMP=0.40:1.15:45:300
#                Filtri FC (non-DTS): Highpass 110Hz, Lowpass 6800Hz, Compressore, Softclipper.
#
# CODEC SUPPORTATI:
...
#
# ESEMPI:
#   ./clearvoice_preset.sh --serie eac3 384k "Serie.mkv"  # Singolo file, preset serie, DD+ 384k
#   ./clearvoice_preset.sh --film dts 768k                # Tutti i .mkv in dir, preset film, DTS 768k
#   ./clearvoice_preset.sh --cartoni ac3 448k             # Tutti i .mkv, preset cartoni, AC3 448k
#
# ELABORAZIONE:
...
#   - Se la GPU supporta l’accelerazione hardware, lo script utilizza -hwaccel cuda.
...
# PIPELINE PER FILM
  ADV_FILTER=$(cat <<EOF | tr -d '\n'
    ...
    [FC]highpass=f=110,lowpass=f=8000,volume=${VOICE_VOL_ADJ},${COMPRESSOR_SETTINGS},${SOFTCLIP_SETTINGS}[center];
...
# Calcoli aritmetici con awk per mantenere la parametrizzazione
  VOICE_VOL_ADJ=$(awk "BEGIN {print $VOICE_VOL + 0.8}") # Aumento per la voce
  FRONT_VOL_ADJ=$(awk "BEGIN {print $FRONT_VOL - 0.2}") # Leggera riduzione per i frontali
  LFE_VOL_ADJ=$(awk "BEGIN {print $LFE_VOL * 0.9}")     # Riduzione LFE per serie
  SURROUND_VOL_ADJ=$(awk "BEGIN {print $SURROUND_VOL * 0.95}") # Leggera riduzione surround per serie

# PIPELINE PER SERIE TV
  ADV_FILTER=$(cat <<EOF | tr -d '\n'
    [0:a]channelmap=channel_layout=5.1[audio5dot1];
    [audio5dot1]channelsplit=channel_layout=5.1[FL][FR][FC][LFE][BL][BR];  
    [FC]highpass=f=100,lowpass=f=7800,volume=${VOICE_VOL_ADJ},${COMPRESSOR_SETTINGS},${SOFTCLIP_SETTINGS}[center];
    [FL]volume=${FRONT_VOL_ADJ}[left];
    [FR]volume=${FRONT_VOL_ADJ}[right];
    [LFE]volume=${LFE_VOL_ADJ}[bass];
    [BL]volume=${SURROUND_VOL_ADJ}[surroundL];
    [BR]volume=${SURROUND_VOL_ADJ}[surroundR];
    [left][right][center][bass][surroundL][surroundR]join=inputs=6:channel_layout=5.1[out]
EOF
...
# Calcoli aritmetici con awk per mantenere la parametrizzazione
  VOICE_VOL_ADJ=$(awk "BEGIN {print $VOICE_VOL + 0.3}") # Leggero aumento per la voce
  FRONT_VOL_ADJ=$(awk "BEGIN {print $FRONT_VOL - 0.1}") # Leggera riduzione per i frontali (risulta 0.9)
  SURROUND_VOL_ADJ=$SURROUND_VOL    # Volume surround invariato per cartoni

# PIPELINE PER CARTONI/MUSICAL
  ADV_FILTER=$(cat <<EOF | tr -d '\n'
    [0:a]channelmap=channel_layout=5.1[audio5dot1];
    [audio5dot1]channelsplit=channel_layout=5.1[FL][FR][FC][LFE][BL][BR];
    [FC]highpass=f=110,lowpass=f=6800,volume=${VOICE_VOL_ADJ},${COMPRESSOR_SETTINGS},${SOFTCLIP_SETTINGS}[center];
    [FL]volume=${FRONT_VOL_ADJ}[left];
    [FR]volume=${FRONT_VOL_ADJ}[right];
    [LFE]volume=${LFE_VOL}[bass];
    [BL]volume=${SURROUND_VOL_ADJ}[surroundL];
...
# Regolazione LFE specifica per DTS
  if [[ "$PRESET" == "film" ]]; then
    ...
    LFE_BOOST=0.92 
  elif [[ "$PRESET" == "cartoni" ]]; then # caso esplicito per cartoni
    LFE_BOOST=1.02 
  fi
...
# Determina il volume dei canali frontali per DTS in base al PRESET
  ...
  if [[ "$PRESET" == "film" ]]; then
    ...
    DTS_FRONT_FL_VOLUME="0.7"
    DTS_FRONT_FR_VOLUME="0.7"
  elif [[ "$PRESET" == "cartoni" ]]; then
    DTS_FRONT_FL_VOLUME="0.8" 
    DTS_FRONT_FR_VOLUME="0.8"
  fi
...
# Riduce il volume della voce per film DTS per mitigare vibrazioni
    DTS_EFFECTIVE_VOICE_VOL=$(awk "BEGIN {print $VOICE_VOL - 0.5}")
  elif [[ "$PRESET" == "serie" ]]; then
# Alza il volume della voce per serie DTS
    DTS_EFFECTIVE_VOICE_VOL=$(awk "BEGIN {print $VOICE_VOL + 0.5}") 
  elif [[ "$PRESET" == "cartoni" ]]; then
# Aggiusta il volume della voce per cartoni DTS (esempio: leggero aumento)
    DTS_EFFECTIVE_VOICE_VOL=$(awk "BEGIN {print $VOICE_VOL + 0.3}")
  fi
...
# Applica i filtri e ricodifica la traccia mantenendo l'originale
  ffmpeg -hwaccel cuda -y -hide_banner -avoid_negative_ts make_zero -fflags +genpts \
