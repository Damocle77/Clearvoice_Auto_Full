...
# --- Segmentazione audio (adatta il numero/finestra di analisi al file) ---

if [ "$DURATION" -le 1800 ]; then
    NUM_SEGMENTS=3; SEGMENT_DUR=180
elif [ "$DURATION" -le 3600 ]; then
    NUM_SEGMENTS=4; SEGMENT_DUR=200
elif [ "$DURATION" -le 7200 ]; then
    NUM_SEGMENTS=5; SEGMENT_DUR=210
else
    NUM_SEGMENTS=6; SEGMENT_DUR=220
fi
...
echo -e "\033[1;36m[Attendere]\033[0m Analisi spettrale in corso, potrebbero servire diversi minuti in base al file..."
...
# --- EQ LFE/Voice/Surround ---

# Voice Equalizer (FC + EQ Voce italiana)
EQ_VOICE="[FC]highpass=f=${FC_HPF},equalizer=f=1000:w=1.2:g=2,equalizer=f=2500:w=1.2:g=1,volume=${VOICE_BOOST}[FCout];"

# LFE Equalizer (punch, cut, oppure binge)
if [[ "$SUB_EQ_MODE" == "punch" ]]; then
    EQ_SUB="[LFE]highpass=f=40,equalizer=f=80:w=1.0:g=4,equalizer=f=120:w=1.2:g=-1,volume=${LFE_REDUCTION}[LFEout];"
...
${EQ_SUB} \
${EQ_SURROUND} \
[FLout][FRout][FCout][LFEout][SLout][SRout]join=inputs=6:channel_layout=5.1,volume=1.0[clearvoice]"

# Dichiarazione array audio
declare -a AUDIO_ARGS=()
if [[ "${INCLUDE_ORIGINAL,,}" =~ ^(no|n|false)$ ]]; then
    # Ricava la lingua dalla traccia audio (default: ita)
