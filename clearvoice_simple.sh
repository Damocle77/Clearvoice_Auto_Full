...
#   - Raccomandazioni bitrate: TV 448k+ (ac3) | 386k+ (eac3) | Film 640k (ac3) o 768k (eac3)
...
if [ -f "$OUTPUT_FILE" ]; then
    ...
    OUTPUT_FILE="$INPUT_DIR/${INPUT_BASENAME%.*}_clearvoice_${TIMESTAMP}.mkv"
...
for ((i=0; i<${#LUFS_ARR[@]}; i++)); do
    ...
    # Verifica valori numerici validi prima di aggiungere
    if [[ "${LUFS_ARR[$i]}" =~ ^-?[0-9]+(\.[0-9]+)?$ ]]; then
        LUFS_SUM=$(awk "BEGIN {print $LUFS_SUM+(${LUFS_ARR[$i]})}")
    fi
    if [[ "${LRA_ARR[$i]}" =~ ^-?[0-9]+(\.[0-9]+)?$ ]]; then
    ...
    # Protezione contro divisione per zero e valori non numerici
    LUFS_COUNT=${#LUFS_ARR[@]}
    LRA_COUNT=${#LRA_ARR[@]}
    TP_COUNT=${#TP_ARR[@]}
    ...
    # Calcolo delle medie
    if [ "$LUFS_COUNT" -eq 0 ]; then LUFS=0; else LUFS=$(awk "BEGIN {print $LUFS_SUM/$LUFS_COUNT}"); fi
    if [ "$LRA_COUNT" -eq 0 ]; then LRA=0; else LRA=$(awk "BEGIN {print $LRA_SUM/$LRA_COUNT}"); fi
    if [ "$TP_COUNT" -eq 0 ]; then TP=0; else TP=$(awk "BEGIN {print $TP_SUM/$TP_COUNT}"); fi
