...
# =========================================================================
# VOCE (Center):
#   VOICE_VOL = 4.56   	# circa +12.6 dB sul centro (intelligibile anche a volumi bassi)
...
VOICE_VOL=4.56        # circa +15dB su dialoghi
LFE_VOL=0.58          # circa -4.7dB su subwoofer
LFE_LIMIT=0.75        # limitazione ceiling LFE (0-1)
FRONT_VOL=1.10        # +1dB sui frontali L/R (no bass-shelf)
SURROUND_VOL=2.25     # +6dB sui surround (pure volume boost)
...
[fc]highpass=f=110,\
  compand=attacks=3:decays=120:points=-80/-80|-52/-36|-25/-13|0/0:soft-knee=6:gain=5.6,\
  acompressor=threshold=-21dB:ratio=4:attack=8:release=120:knee=10dB,\
  alimiter=limit=0.92:attack=3:release=50,\
  equalizer=f=6000:t=q:w=1.6:g=-2.8,\
  equalizer=f=4000:t=q:w=1.2:g=-0.8,\
  equalizer=f=3500:t=q:w=1.2:g=+1.0,\
  equalizer=f=2000:t=q:w=1.2:g=+1.5,\
  equalizer=f=1500:t=q:w=1.0:g=+1.2,\
  equalizer=f=250:t=q:w=1.2:g=+0.9,\
  equalizer=f=180:t=q:w=1.2:g=+0.3,\
  treble=g=1.8f,\
...
[lfe]highpass=f=28:t=q:w=1.2,\
  ...
  volume=${LFE_VOL},\
  alimiter=limit=${LFE_LIMIT}:attack=3:release=250[lfe_p];

[fl]adelay=delays=${FL_DELAY}:all=1,highpass=f=80,volume=${FRONT_VOL}[fl_p];
[fr]adelay=delays=${FR_DELAY}:all=1,highpass=f=80,volume=${FRONT_VOL}[fr_p];

[sl]adelay=delays=${SL_DELAY}:all=1,highpass=f=120,volume=${SURROUND_VOL}[sl_p];
[sr]adelay=delays=${SR_DELAY}:all=1,highpass=f=120,volume=${SURROUND_VOL}[sr_p];
