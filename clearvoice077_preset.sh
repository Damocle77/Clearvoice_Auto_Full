...
set_preset_params() {
    case "$PRESET" in
        ...
        serie)
            ...
            VOICE_COMP="0.35:1.15:50:380"  # Compressione più delicata
...
[left][right][center][bass][surroundL][surroundR]join=inputs=6:channel_layout=5.1[joined];
[joined]aresample=48000:resampler=soxr:precision=28,aformat=sample_fmts=s32:channel_layouts=5.1[out]
EOF
)
    fi
...
process() {
    ...
    if [[ $exit_code -eq 0 ]]; then
        # PROCESSED_FILES+=("$input_file") // Rimossa
        # ((PROCESSED_COUNT++)) // Rimossa
...
print_summary() {
    ...
    # Rimossa tutta la logica che dipendeva da PROCESSED_COUNT, FAILED_COUNT, e PROCESSED_FILES
    # come l'elenco dei file, il tempo medio, le ottimizzazioni specifiche, i consigli e i messaggi di verifica.
    echo "═══════════════════════════════════════════════════════════════════════════════"
}
