...
#                Filtri FC: Highpass 90Hz, Lowpass 7000Hz, Compressore, Softclipper. 
#
...
#
# NOTE:
#   - Per serie TV si consiglia di non scendere sotto i 384k
#   - Hardware acceleration CUDA è abilitata di default
#   - Conserva video originale, tracce audio secondarie e sottotitoli
# -----------------------------------------------------------------------------------------------
...
case "$PRESET" in
  film)
    ...
    VOICE_VOL=7.4; LFE_VOL=0.30; SURROUND_VOL=3.5; # VOICE_VOL increased from 6.3
    # Compressione leggermente più aggressiva per film ad alto dinamismo
    VOICE_COMP="0.48:1.15:45:450";;
  serie)
    # Pipeline parametri serie
    VOICE_VOL=7.3; LFE_VOL=0.29; SURROUND_VOL=3.4; # VOICE_VOL increased from 6.4
    # Compressione media bilanciata per dialoghi TV, resa leggermente più dolce
    VOICE_COMP="0.44:1.18:50:400";; # Adjusted from "0.42:1.2:45:380"
  cartoni)
    # Pipeline parametri cartoni con compressione leggera
    VOICE_VOL=6.5; LFE_VOL=0.31; SURROUND_VOL=3.6;
    # Compressione molto leggera, solo per uniformare
    VOICE_COMP="0.47:1.12:40:300";;
  *)
...
# PIPELINE PER FILM (AC3)
  ...
  [FC]highpass=f=110,lowpass=f=7500,volume=${VOICE_VOL},${COMPRESSOR_SETTINGS},${SOFTCLIP_SETTINGS}[center];
...
# PIPELINE PER SERIE TV
  ...
  [0:a]channelmap=channel_layout=5.1[audio5dot1];
  [audio5dot1]channelsplit=channel_layout=5.1[FL][FR][FC][LFE][BL][BR];
  [FC]highpass=f=90,lowpass=f=7000,volume=${VOICE_VOL},${COMPRESSOR_SETTINGS},${SOFTCLIP_SETTINGS}[center];
  [FL]volume=0.8[left];
  [FR]volume=0.8[right];
...
# Calcoli aritmetici con awk per mantenere la parametrizzazione
  ...
  FRONT_VOL_ADJ=$(awk "BEGIN {print $FRONT_VOL - 0.1}") 
  SURROUND_VOL_ADJ=$SURROUND_VOL  
...
# Regolazione LFE specifica per DTS
  if [[ "$PRESET" == "film" ]]; then
    LFE_BOOST=0.95 # Originale 1.05, ridotto per "2 punti"
  elif [[ "$PRESET" == "serie" ]]; then
    LFE_BOOST=0.97 # Originale 1.02 (da 'else'), ridotto per "1 punto"
...
# Determina il volume dei canali frontali per DTS in base al PRESET
  ...
  if [[ "$PRESET" == "film" ]]; then
    ...
    DTS_FRONT_FL_VOLUME="1.0"
    DTS_FRONT_FR_VOLUME="1.0"
  else 
    DTS_FRONT_FL_VOLUME=$(awk "BEGIN {print $FRONT_VOL - 0.3}")
    DTS_FRONT_FR_VOLUME=$(awk "BEGIN {print $FRONT_VOL - 0.3}")
  fi
...
# Riduce il volume della voce per film DTS per mitigare vibrazioni
    DTS_EFFECTIVE_VOICE_VOL=$(awk "BEGIN {print $VOICE_VOL - 0.6}")
  fi
...
# Aggiunge Lowpass e EQ specifici per preset DTS
  if [[ "$PRESET" == "film" ]]; then 
      ...
      FC_DTS_FILTERS+=",equalizer=f=300:t=h:w=1.0:g=-2.0,equalizer=f=3000:t=h:w=1.0:g=+0.5"
  elif [[ "$PRESET" == "serie" ]]; then 
      FC_DTS_FILTERS="${FC_DTS_FILTERS_BASE},lowpass=f=7000"
      FC_DTS_FILTERS+=",equalizer=f=250:t=h:w=1.0:g=-1.5,equalizer=f=2800:t=h:w=1.0:g=+1.2"
  elif [[ "$PRESET" == "cartoni" ]]; then
...
FILTER_PRE=$(cat <<EOF | tr -d '\n'
  ...
  [BL]volume=$(awk "BEGIN {print $SURROUND_VOL * 0.8}")[surroundL];
  [BR]volume=$(awk "BEGIN {print $SURROUND_VOL * 0.8}")[surroundR];
EOF
)
# Aggiunge il join finale per DTS  
  ADV_FILTER="${FILTER_PRE}  [left][right][center][bass][surroundL][surroundR]join=inputs=6:channel_layout=5.1:map=0.0-FL|1.0-FR|2.0-FC|3.0-LFE|4.0-BL|5.0-BR[joined]; [joined]aresample=48000,asetnsamples=n=1152:p=0,aformat=sample_fmts=s32:channel_layouts=5.1[out]"
fi
...
# Applica la serie di filtri e ricodifica la traccia processata mantenendo l'originale
