...
#                Parametri: VOICE_VOL=8.6, LFE=0.23, SURR=3.4, COMP=0.40:1.15:60:380
...
#                Parametri: VOICE_VOL=5.8, LFE=0.23, SURR=3.4, COMP=0.42:1.28:20:320
#                Filtri FC: Highpass 180Hz, Lowpass 6000Hz, Equalizzatore dialoghi integrato
...
#                Parametri: VOICE_VOL=8.4, LFE=0.23, SURR=3.5, COMP=0.40:1.15:50:330
...
#
# OPZIONI AUDIO:
#   --replace : Sostituisce completamente la traccia originale (solo ClearVoice)
#   (default) : Aggiunge ClearVoice come primaria + mantiene originale come secondaria
#
# ESEMPI D'USO:
#   ./clearvoice080_preset.sh --serie eac3 320k *.mkv           # ClearVoice + originale
#   ./clearvoice080_preset.sh --film dts 768k --replace *.mkv   # Solo ClearVoice
#   ./clearvoice080_preset.sh --cartoni --replace *.mkv         # Solo ClearVoice cartoni
#   ./clearvoice080_preset.sh --tv *.mkv                        # ClearVoice + originale + EQ
#   ./clearvoice080_preset.sh --serie /path/to/series/          # Cartella serie completa
#
...
#   ✓ Scelta flessibile gestione tracce audio: ClearVoice + originale oppure solo ClearVoice
#   ✓ Metadata ottimizzati per tracce multiple o singola secondo modalità scelta
...
#
# NOVITÀ v0.80:
#   - Modalità flessibile gestione tracce audio: solo ClearVoice o ClearVoice + originale
#   - Parametro --replace per controllo completo output audio (sostituisce --replace-audio)
...
#   - Calcoli numerici sicuri con fallback automatico (mantenuti da v0.79)
#   - Validazione robusta parametri compressione dinamica (mantenuti da v0.79)
#   - Fix gestione equalizzatore specifico per preset TV (mantenuti da v0.79)
#   - Processing sequenziale ottimizzato per stabilità massima (mantenuti da v0.79)
#   - Validazione input avanzata con analisi formati audio dettagliata (mantenuti da v0.79)
#
...
BR=""
INPUTS=()
REPLACE_AUDIO=false  # Flag per controllo tracce audio
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --replace) REPLACE_AUDIO=true; shift;;
    -h|--help)
      cat << 'EOF'

CLEARVOICE 0.80 - Ottimizzazione Audio 5.1 per LG Meridian SP7

USO: ./clearvoice080_preset.sh [PRESET] [CODEC] [BITRATE] [--replace] [FILES...]

PRESET:
  --film     Cinema/Action (VOICE:8.5, LFE:0.23, moderato)
  --serie    Serie TV/Dialoghi (VOICE:8.6, LFE:0.23, intelligibilità max)
  --tv       Materiale problematico (VOICE:5.8, LFE:0.23 conservativo + EQ intelligibile)
  --cartoni  Animazione (VOICE:8.4, LFE:0.23, preserva musica)
...
ESEMPI:
  ./clearvoice080_preset.sh --serie *.mkv                    # ClearVoice + originale
  ./clearvoice080_preset.sh --film dts 768k --replace *.mkv  # Solo ClearVoice
  ./clearvoice080_preset.sh --cartoni --replace *.mkv        # Solo ClearVoice cartoni
  ./clearvoice080_preset.sh --tv *.mkv                       # ClearVoice + originale + EQ
...
CONFIGURAZIONE TRACCE AUDIO:
  Default: ClearVoice (primaria) + Audio originale (secondaria)
  --replace: Solo traccia ClearVoice ottimizzata

MIGLIORAMENTI QUALITÀ v0.80:
  ✓ Scelta flessibile gestione tracce audio originali
  ✓ Metadata ottimizzati per tracce multiple o singola
  ✓ Calcoli numerici sicuri con fallback automatico
  ✓ Validazione robusta parametri compressione dinamica
  ✓ Equalizzatore intelligibile per preset TV
...
# -----------------------------------------------------------------------------------------------
#  PROCESSING CON GESTIONE ERRORI AVANZATA E CONTROLLO TRACCE AUDIO
# -----------------------------------------------------------------------------------------------
...
process() {
    ...
    # Processing FFmpeg con comando costruito correttamente
    if [[ "$REPLACE_AUDIO" == true ]]; then
        # Modalità sostituzione: solo traccia ClearVoice
        echo "🎬 Processing (SOLO ClearVoice): $(basename "$input_file") [Preset: $PRESET]"
        ...
        # Modalità default: ClearVoice primaria + originale secondaria
        echo "🎬 Processing (ClearVoice + Originale): $(basename "$input_file") [Preset: $PRESET]"
        
        ffmpeg -hwaccel auto -y -hide_banner -avoid_negative_ts make_zero \
            -threads "$thread_count" -filter_threads "$thread_count" -thread_queue_size 512 \
            -i "$input_file" -filter_complex "$LOCAL_FILTER" \
            -map "[out]" -map 0:a -map 0:v -map 0:s? \
            -metadata:s:a:0 title="$TITLE" -metadata:s:a:0 language=ita -disposition:a:0 default \
            -metadata:s:a:1 title="Audio Originale" -metadata:s:a:1 language=ita -disposition:a:1 0 \
    ...
    if [[ $? -eq 0 ]]; then
        ...
        if [[ "$REPLACE_AUDIO" == true ]]; then
            echo "✅ Completato (SOLO ClearVoice) in ${PROCESSING_TIME}s: $(basename "$out")"
        else
            echo "✅ Completato (ClearVoice + Originale) in ${PROCESSING_TIME}s: $(basename "$out")"
        fi
        return 0
    else
...
print_summary() {
    ...
    if [[ "$REPLACE_AUDIO" == true ]]; then
        echo "   • Modalità: Solo traccia ClearVoice ottimizzata"
    else
        echo "   • Modalità: ClearVoice primaria + Audio originale secondaria"
    fi
...
if [[ "$REPLACE_AUDIO" == true ]]; then
    echo "🔄 Modalità: Sostituzione completa traccia audio (solo ClearVoice)"
else
    echo "🔄 Modalità: ClearVoice primaria + conservazione audio originale"
fi

if ! validate_inputs; then
    echo ""
    echo "🆘 HELP: ClearVoice richiede tracce audio 5.1"
    echo "   Usa i comandi di conversione indicati o ffMediaMaster, poi rilancia ClearVoice"
    exit 1
fi
